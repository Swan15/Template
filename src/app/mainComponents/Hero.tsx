...
export default function Hero({ user }: { user: User | null }) {
    return (
        <section className='flex flex-col items-center  gap-8 pb-8 pt-16  text-center'>
            ...
            <p className='max-w-2xl text-lg font-bold tracking-tight opacity-70'>
                Never setup another email server for your MVP again. Get started
                with EmailMe and get project notifications right away.
            </p>
            <div className='mb-4  flex justify-center gap-x-6'>
                {!user && (
                    <a
                        ...
                        className='btn btn-lg w-36'
                    >
                        Get started
                    </a>
                )}
                {user && (
                    <Link href='/dashboard' className='btn btn-lg w-36'>
                        Dashboard
                    </Link>
                )}
                <Link href='/docs' className='btn btn-lg w-36'>
                    Docs
                </Link>
            </div>
        </section>
